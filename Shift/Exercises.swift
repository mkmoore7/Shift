...
class Exercises: UIViewController {
    ...
    var y = 0.0
    var radius:Double?
    var acceleration : CMAcceleration = CMAcceleration(x: 0.0,y: 0.0,z: 0.0)
    ...
    func movePlayer(acc: CMAcceleration){
        ...
        //need a bounding box
//        let x:Double = Double(player!.position.x) + ((acc.x - self.acceleration.x)/0.2 * 50.0)
//        let y:Double = Double(player!.position.y) + ((acc.y - self.acceleration.y)/0.2 * 50.0)
//        
...
//        self.acceleration.y = acc.y
       ...
       // let xDiff:Double = Double(player!.position.x) + ((acc.x - self.acceleration.x)/0.2 * 50.0)
        //let yDiff:Double = Double(player!.position.y) + ((acc.y - self.acceleration.y)/0.2 * 50.0)
        ...
        x = x + Double(20 * acc.x)
        y = y - Double(20 * acc.z)
        if( pow((Double(x) - Double(self.playerRestPosition!.x)), 2.0) + pow(Double(y) - Double(self.playerRestPosition!.y), 2.0) <= (radius! * radius!)){
